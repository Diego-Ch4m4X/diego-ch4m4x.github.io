# Diego Ch4m4X — Hub Técnico

Este é o ponto central dos meus projetos, guias, ferramentas e estudos publicados no GitHub Pages.

O conteúdo reúne temas ligados a **telecomunicações, redes, Linux, automação, desenvolvimento Full Stack, DevOps, NetDev e NetDevOps**, conectando minha experiência em infraestrutura e redes ao aprendizado contínuo em software.

🌐 **Acesse o hub:** [diego-ch4m4x.github.io](https://diego-ch4m4x.github.io/)

---

## Projetos em destaque

| Projeto | Descrição | Acessar |
| --- | --- | --- |
| **ANOTAÇÕES** | Workspace local-first para organização, edição e visualização de arquivos `.txt` e `.md`, com árvore de arquivos, abas, busca, Markdown, personalização e backup JSON. | [Abrir projeto](https://diego-ch4m4x.github.io/ANOTACOES/) · [GitHub](https://github.com/Diego-Ch4m4X/ANOTACOES) |
| **GUIA GIT** | Guia técnico e prático de Git e GitHub, com comandos explicados, cenários reais, branches, conflitos, sincronização, recuperação e laboratórios. | [Abrir guia](https://diego-ch4m4x.github.io/Guia_Git/) · [GitHub](https://github.com/Diego-Ch4m4X/Guia_Git) |
| **GUIA MARKDOWN** | Referência em português para Markdown, incluindo CommonMark, GFM, GitHub, Mermaid, Math/LaTeX, HTML, Front Matter e boas práticas de documentação técnica. | [Abrir guia](https://diego-ch4m4x.github.io/guia-markdown-definitivo/) · [GitHub](https://github.com/Diego-Ch4m4X/guia-markdown-definitivo) |
| **LINUX DISTRO ADVISOR** | Guia técnico e quiz interativo para comparar distribuições Linux por workload, ciclo de vida, segurança, hardware e operação, com foco em critérios verificáveis. | [Abrir projeto](https://diego-ch4m4x.github.io/Linux_Distro_Advisor/) · [GitHub](https://github.com/Diego-Ch4m4X/Linux_Distro_Advisor) |

---

## Sobre mim

Atuo na área de **telecomunicações e redes** e mantenho interesse constante por infraestrutura, Linux, automação e desenvolvimento de software.

Atualmente amplio meus conhecimentos em **programação, desenvolvimento Full Stack e práticas DevOps**, buscando aproximar redes e software e evoluir cada vez mais nas áreas de **NetDev e NetDevOps**.

### Áreas de interesse

- Telecomunicações e redes
- Linux
- Automação
- Desenvolvimento Full Stack
- DevOps
- NetDev
- NetDevOps

---

## Propósito

Este hub acompanha minha evolução técnica e reúne projetos que surgem de estudo, experimentação e aplicação prática.

A ideia é transformar conhecimento em algo que possa ser **consultado, testado, melhorado e compartilhado**.

---

## Links

- [GitHub](https://github.com/Diego-Ch4m4X)
- [LinkedIn](https://www.linkedin.com/in/diegodsl)
- [Hub técnico](https://diego-ch4m4x.github.io/)

---

© 2026 Diego Ch4m4X
